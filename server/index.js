...
app.get('/api/dashboard', (req, res) => {
  ...
  const openOrders = orders.filter(o => ['pending', 'confirmed', 'in-transit'].includes(o.status)).length;
  const criticalAlerts = alerts.filter(a => !a.resolved && a.type === 'critical').length;
  const avgDeliveryRate = Math.round(suppliers.filter(s => s.status === 'active').reduce((sum, s) => sum + s.onTimeDelivery, 0) / activeSuppliers);
...
app.get('/api/orders', (req, res) => {
  ...
  let result = [...orders];
  if (search) result = result.filter(o => o.id.toLowerCase().includes(search.toLowerCase()) || o.supplierName.toLowerCase().includes(search.toLowerCase()));
